...
void main() {
  group('Span Links and Status', () {
    ...
    test('creates span with links at creation time', () {
      ...
      final span1 = tracer.createSpan('span-1');
      final span2 = tracer.createSpan('span-2');

      final attributes = {'key': 'value'}.toAttributes();

      // Create a span with links to both spans
      final linkedSpan = tracer.createSpan(
        'linked-span',
        links: [
          OTelAPI.createSpanLink(span1.spanContext, attributes),
          OTelAPI.createSpanLink(span2.spanContext, attributes),
        ],
      );
      ...
      expect(linkedSpan.spanLinks?.first.attributes.get('key'), equals('value'));
    });

    test('adds links after span creation', () {
      final span1 = tracer.createSpan('span-1');
      final mainSpan = tracer.createSpan('main-span');
      ...
      expect(mainSpan.spanLinks?.first.attributes.get('key'), equals('value'));
    });

    test('preserves link order', () {
      final span1 = tracer.createSpan('span-1');
      final span2 = tracer.createSpan('span-2');
      final span3 = tracer.createSpan('span-3');
    ...
    test('handles links with empty trace IDs or span IDs', () {
      ...
      final span = tracer.createSpan(
        'test-span',
        links: [OTelAPI.createSpanLink(invalidContext, attributes)],
      );

      expect(span.spanLinks, hasLength(1));
      expect(span.spanLinks?.first.spanContext, equals(invalidContext));
      expect(span.spanLinks?.first.attributes.get('key'), equals('value'));
    });

    test('isRecording behavior', () {
      final span = tracer.createSpan('test-span');
      ...
      span.addEvent(OTelAPI.createSpanEvent('test-event'));
